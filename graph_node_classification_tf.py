...
def create_ffn(
    ...
    for units in hidden_units:
        ...
        fnn_layers.append(keras.layers.Dense(units, activation=keras.activations.gelu))
...
class GraphConvLayer(keras.Layer):
    ...
    def call(self, inputs: Tuple[tf.Tensor, tf.Tensor, tf.Tensor]) -> tf.Tensor:
        ...
        if neighbour_representations.ndim == 1:
            neighbour_representations = ops.expand_dims(
                neighbour_representations, axis=-1
            )
...
class GNNNodeClassifier(keras.Model):
    ...
    def call(self, input_node_idx):
        ...
        if node_embds.ndim == 1:
            node_embds = ops.expand_dims(node_embds, axis=-1)
...
def main():
    ...
    _ = model(keras.ops.convert_to_tensor([1, 10, 100]))
    ...
    early_callback = keras.callbacks.EarlyStopping(
        monitor="val_acc", patience=50, restore_best_weights=True, mode="max"
    )
